...
import BaySelectionModal from './BaySelectionModal';
...
type WallElement = Record<string, unknown>;
...
const WallOptions: React.FC<WallOptionsProps> = ({ 
  ...
  const [currentPositionField, setCurrentPositionField] = useState<{wall: string, index: number, type: string, currentBay: string} | null>(null);
  ...
  const handleOpenBayModal = (wall: string, index: number, type: string, currentBay: string) => {
    setCurrentPositionField({wall, index, type, currentBay});
    setShowBayModal(true);
  };
  
  const handleBaySelected = (bay: string) => {
    if (currentPositionField) {
      const { wall, index, type } = currentPositionField;
      
      if (type === 'door') {
        updateDoor(wall, index, 'position.bay', bay);
      } else if (type === 'window') {
        updateWindow(wall, index, 'position.bay', bay);
      } else if (type === 'bayDoor') {
        updateBayDoor(wall, index, 'position.bay', bay); 
      } else if (type === 'opening') {
        updateOpening(wall, index, 'position.bay', bay);
      }
  ...
  const PositioningOptions = ({ 
    ...
    wall: string,
    index?: number,
    type?: 'door' | 'window' | 'bayDoor' | 'opening'
  }) => (
          ...
          >
            ...
            <Form.Group className="mt-2">
              ...
              <div className="d-flex align-items-center">
                <div className="me-2 fw-bold">Bay {position.bay}</div>
                <Button
                  variant="outline-primary"
                  size="sm"
                  onClick={() => handleOpenBayModal(wall, index, type, position.bay)}
                >
                  Pick Bay
                </Button>
              </div>
                    ...
                    {wallData.doors.map((door, index) => (
                      <Card key={door.id} className="mb-3 position-card">
                        <Card.Body>
                          ...
                          <hr />
                          <h6>Positioning</h6>                          <PositioningOptions 
